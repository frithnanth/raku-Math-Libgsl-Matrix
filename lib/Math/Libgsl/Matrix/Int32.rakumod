...
unit class Math::Libgsl::Matrix::Int32:ver<0.5.0>:auth<zef:FRITH>;
...
multi method list(Math::Libgsl::Matrix::Int32: --> List) {
  my gsl_vector_int $v = gsl_vector_int_calloc($!matrix.size2);
  LEAVE { gsl_vector_int_free($v) }

  do for ^$!matrix.size1 {
    gsl_matrix_int_get_row($v, $!matrix, $_);
    (^$v.size).map( { gsl_vector_int_get($v, $_) } ).eager;
  }
}
multi method gist(Math::Libgsl::Matrix::Int32: --> Str) {
  ...
  do for ^$size1 {
    gsl_matrix_int_get_row($v, $!matrix, $_);
    '(' ~ (^$size2).map({ gsl_vector_int_get($v, $_) }).Str ~ "$ellip1)\n";
  } ~ "$ellip2)"
}
multi method Str(Math::Libgsl::Matrix::Int32: --> Str) { self.list.join(' ') }
# Accessors

...
unit class Math::Libgsl::Vector::Int32:ver<0.5.0>:auth<zef:FRITH>;
...
multi method list(Math::Libgsl::Vector::Int32: --> List) { (^$!vector.size).map({ gsl_vector_int_get($!vector, $_) }).List }
multi method gist(Math::Libgsl::Vector::Int32: --> Str) {
  my ($size, $ellip);
  if $!vector.size > 100 {
    $size = 100;
    $ellip = ' ...';
  ...
  '(' ~ (^$size).map({ gsl_vector_int_get($!vector, $_) }).Str ~ "$ellip)";
}
multi method Str(Math::Libgsl::Vector::Int32: --> Str) {
  (^$!vector.size).map({ gsl_vector_int_get($!vector, $_) })».Str.join(' ')
}

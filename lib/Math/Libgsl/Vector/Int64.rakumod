...
unit class Math::Libgsl::Vector::Int64:ver<0.5.0>:auth<zef:FRITH>;
...
class View {
  ...
  method subvector(Math::Libgsl::Vector::Int64 $v, size_t $offset where * < $v.vector.size, size_t $n --> Math::Libgsl::Vector::Int64) {
    fail X::Libgsl.new: errno => GSL_EDOM, error => "Subvector index out of bound" if $offset + $n > $v.vector.size;
    Math::Libgsl::Vector::Int64.new: vector => mgsl_vector_long_subvector($!view, $v.vector, $offset, $n);
  }
  method subvector-stride(Math::Libgsl::Vector::Int64 $v, size_t $offset where * < $v.vector.size, size_t $stride, size_t $n --> Math::Libgsl::Vector::Int64) {
    fail X::Libgsl.new: errno => GSL_EDOM, error => "Subvector index out of bound" if $offset + $n > $v.vector.size;
    Math::Libgsl::Vector::Int64.new: vector => mgsl_vector_long_subvector_with_stride($!view, $v.vector, $offset, $stride, $n);
  }
  method array($array --> Math::Libgsl::Vector::Int64) {
    Math::Libgsl::Vector::Int64.new: vector => mgsl_vector_long_view_array($!view, $array, $array.list.elems);
  }
  method array-stride($array, size_t $stride --> Math::Libgsl::Vector::Int64) {
    Math::Libgsl::Vector::Int64.new: vector => mgsl_vector_long_view_array_with_stride($!view, $array, $stride, $array.list.elems);
  }
}
...
multi method list(Math::Libgsl::Vector::Int64: --> List) { (^$!vector.size).map({ gsl_vector_long_get($!vector, $_) }).List }
multi method gist(Math::Libgsl::Vector::Int64: --> Str) {
  my ($size, $ellip);
  if $!vector.size > 100 {
    $size = 100;
    $ellip = ' ...';
  ...
  '(' ~ (^$size).map({ gsl_vector_long_get($!vector, $_) }).Str ~ "$ellip)";
}
multi method Str(Math::Libgsl::Vector::Int64: --> Str) {
  (^$!vector.size).map({ gsl_vector_long_get($!vector, $_) })».Str.join(' ')
}
...
# View
sub int64-prepvec(*@array) is export {
  my CArray[int64] $array .= new: @array».Int;
}
sub int64-array-vec(Block $bl, *@data) is export {
  my CArray[int64] $carray .= new: @data».Int;
  my Math::Libgsl::Vector::Int64::View $vv .= new;
  my $v = $vv.array($carray);
  $bl($v);
}
sub int64-array-stride-vec(Block $bl, size_t $stride, *@data) is export {
  my CArray[int64] $carray .= new: @data».Int;
  my Math::Libgsl::Vector::Int64::View $vv .= new;

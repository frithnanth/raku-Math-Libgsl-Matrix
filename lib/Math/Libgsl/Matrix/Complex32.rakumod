...
unit class Math::Libgsl::Matrix::Complex32:ver<0.1.3>:auth<cpan:FRITH>;
...
sub mat-view-array(Math::Libgsl::Matrix::Complex32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export(:withsub) {
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::Complex32.new: matrix => mgsl_matrix_complex_float_view_array($mv.view, $a, @array.shape[0], (@array.shape[1] / 2).Int);
}
sub mat-view-array-tda(Math::Libgsl::Matrix::Complex32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export(:withsub) {
  fail X::Libgsl.new: errno => GSL_EDOM, error => "tda out of bound" if $tda < @array.shape[1];
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::Complex32.new: matrix => mgsl_matrix_complex_float_view_array_with_tda($mv.view, $a, @array.shape[0], (@array.shape[1] / 2).Int, $tda);
}

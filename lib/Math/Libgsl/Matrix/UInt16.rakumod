...
unit class Math::Libgsl::Matrix::UInt16:ver<0.1.4>:auth<cpan:FRITH>;
...
method tricpy(Math::Libgsl::Matrix::UInt16 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_ushort_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }
...
method transpose-tricpy(Math::Libgsl::Matrix::UInt16 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_ushort_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }

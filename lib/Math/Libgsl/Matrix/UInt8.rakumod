...
unit class Math::Libgsl::Matrix::UInt8:ver<0.1.3>:auth<cpan:FRITH>;
...
sub mat-view-array(Math::Libgsl::Matrix::UInt8::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export(:withsub) {
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::UInt8.new: matrix => mgsl_matrix_uchar_view_array($mv.view, $a, @array.shape[0], @array.shape[1]);
}
sub mat-view-array-tda(Math::Libgsl::Matrix::UInt8::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export(:withsub) {
  fail X::Libgsl.new: errno => GSL_EDOM, error => "tda out of bound" if $tda < @array.shape[1];
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::UInt8.new: matrix => mgsl_matrix_uchar_view_array_with_tda($mv.view, $a, @array.shape[0], @array.shape[1], $tda);
}
sub mat-view-vector(Math::Libgsl::Matrix::UInt8::View $mv, Math::Libgsl::Vector::UInt8 $v, size_t $n1, size_t $n2) is export(:withsub) {
  Math::Libgsl::Matrix::UInt8.new: matrix => mgsl_matrix_uchar_view_vector($mv.view, $v.vector, $n1, $n2);
}
sub mat-view-vector-tda(Math::Libgsl::Matrix::UInt8::View $mv, Math::Libgsl::Vector::UInt8 $v, size_t $n1, size_t $n2, size_t $tda) is export(:withsub) {
...
method tricpy(Math::Libgsl::Matrix::UInt8 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_uchar_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }
...
method transpose-tricpy(Math::Libgsl::Matrix::UInt8 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_uchar_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }

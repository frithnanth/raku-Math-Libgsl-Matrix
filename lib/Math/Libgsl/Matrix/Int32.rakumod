...
unit class Math::Libgsl::Matrix::Int32:ver<0.1.2>:auth<cpan:FRITH>;
...
# View
method submatrix(Math::Libgsl::Matrix::Int32::View $mv, size_t $k1 where * < $!matrix.size1, size_t $k2 where * < $!matrix.size2, size_t $n1, size_t $n2) {
...
sub mat-view-array(Math::Libgsl::Matrix::Int32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export(:withsub) {
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::Int32.new: matrix => mgsl_matrix_int_view_array($mv.view, $a, @array.shape[0], @array.shape[1]);
}
sub mat-view-array-tda(Math::Libgsl::Matrix::Int32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export(:withsub) {
  fail X::Libgsl.new: errno => GSL_EDOM, error => "tda out of bound" if $tda < @array.shape[1];
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::Int32.new: matrix => mgsl_matrix_int_view_array_with_tda($mv.view, $a, @array.shape[0], @array.shape[1], $tda);
}
...
method col-view(Math::Libgsl::Vector::Int32::View $vv, size_t $j where * < $!matrix.size2) {
  Math::Libgsl::Vector::Int32.new: vector => mgsl_matrix_int_column($vv.view, $!matrix, $j);
}
method subrow-view(Math::Libgsl::Vector::Int32::View $vv, size_t $i where * < $!matrix.size1, size_t $offset, size_t $n) {
...
method tricpy(Math::Libgsl::Matrix::Int32 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  my $ret = gsl_matrix_int_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
...
method transpose-tricpy(Math::Libgsl::Matrix::Int32 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  fail X::Libgsl.new: errno => GSL_ENOTSQR, error => "Not a square matrix" if $!matrix.size1 ≠ $!matrix.size2;
  my $ret = gsl_matrix_int_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);

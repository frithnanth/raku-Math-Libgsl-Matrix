...
unit class Math::Libgsl::Matrix::UInt32:ver<0.0.6>:auth<cpan:FRITH>;
...
sub mat-view-array(Math::Libgsl::Matrix::UInt32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export {
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::UInt32.new: matrix => mgsl_matrix_uint_view_array($mv.view, $a, @array.shape[0], @array.shape[1]);
}
sub mat-view-array-tda(Math::Libgsl::Matrix::UInt32::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export {
  fail X::Libgsl.new: errno => GSL_EDOM, error => "tda out of bound" if $tda < @array.shape[1];
  my CArray[num64] $a .= new: @array.Array».Num;
  Math::Libgsl::Matrix::UInt32.new: matrix => mgsl_matrix_uint_view_array_with_tda($mv.view, $a, @array.shape[0], @array.shape[1], $tda);
}
sub mat-view-vector(Math::Libgsl::Matrix::UInt32::View $mv, Math::Libgsl::Vector::UInt32 $v, size_t $n1, size_t $n2) is export {
  Math::Libgsl::Matrix::UInt32.new: matrix => mgsl_matrix_uint_view_vector($mv.view, $v.vector, $n1, $n2);
}
sub mat-view-vector-tda(Math::Libgsl::Matrix::UInt32::View $mv, Math::Libgsl::Vector::UInt32 $v, size_t $n1, size_t $n2, size_t $tda) is export {

...
unit class Math::Libgsl::Matrix::Int8:ver<0.6.0>:auth<zef:FRITH>;
...
class View {
  ...
  method submatrix(Math::Libgsl::Matrix::Int8 $m, size_t $k1 where * < $m.size1, size_t $k2 where * < $m.size2, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::Int8) {
    fail X::Libgsl.new: errno => GSL_EDOM, error => "Submatrix indices out of bound" if $k1 + $n1 > $m.size1 || $k2 + $n2 > $m.size2;
    Math::Libgsl::Matrix::Int8.new: matrix => mgsl_matrix_char_submatrix($!view, $m.matrix, $k1, $k2, $n1, $n2);
  }
  method vector(Math::Libgsl::Vector::Int8 $v, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::Int8) {
    Math::Libgsl::Matrix::Int8.new: matrix => mgsl_matrix_char_view_vector($!view, $v.vector, $n1, $n2);
  }
  method vector-tda(Math::Libgsl::Vector::Int8 $v, size_t $n1, size_t $n2, size_t $tda where * > $n2 --> Math::Libgsl::Matrix::Int8) {
    Math::Libgsl::Matrix::Int8.new: matrix => mgsl_matrix_char_view_vector_with_tda($!view, $v.vector, $n1, $n2, $tda);
  }
  method array($array, UInt $size1, UInt $size2 --> Math::Libgsl::Matrix::Int8) {
    Math::Libgsl::Matrix::Int8.new: matrix => mgsl_matrix_char_view_array($!view, $array, $size1, $size2);
  }
  method array-tda($array, UInt $size1, UInt $size2, size_t $tda where * > $size2 --> Math::Libgsl::Matrix::Int8) {
    Math::Libgsl::Matrix::Int8.new: matrix => mgsl_matrix_char_view_array_with_tda($!view, $array, $size1, $size2, $tda);
  }
}
...
multi method list(Math::Libgsl::Matrix::Int8: --> List) {
  my gsl_vector_char $v = gsl_vector_char_calloc($!matrix.size2);
  LEAVE { gsl_vector_char_free($v) }

  do for ^$!matrix.size1 {
    gsl_matrix_char_get_row($v, $!matrix, $_);
    (^$v.size).map( { gsl_vector_char_get($v, $_) } ).eager;
  }
}
multi method gist(Math::Libgsl::Matrix::Int8: --> Str) {
  ...
  do for ^$size1 {
    gsl_matrix_char_get_row($v, $!matrix, $_);
    '(' ~ (^$size2).map({ gsl_vector_char_get($v, $_) }).Str ~ "$ellip1)\n";
  } ~ "$ellip2)"
}
multi method Str(Math::Libgsl::Matrix::Int8: --> Str) { self.list.join(' ') }
# Accessors
...
# View
method row-view(Math::Libgsl::Vector::Int8::View $vv, size_t $i where * < $!matrix.size1 --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_row($vv.view, $!matrix, $i);
}
method col-view(Math::Libgsl::Vector::Int8::View $vv, size_t $j where * < $!matrix.size2 --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_column($vv.view, $!matrix, $j);
}
method subrow-view(Math::Libgsl::Vector::Int8::View $vv, size_t $i where * < $!matrix.size1, size_t $offset, size_t $n --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_subrow($vv.view, $!matrix, $i, $offset, $n);
}
method subcol-view(Math::Libgsl::Vector::Int8::View $vv, size_t $j where * < $!matrix.size2, size_t $offset, size_t $n --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_subcolumn($vv.view, $!matrix, $j, $offset, $n);
}
method diagonal-view(Math::Libgsl::Vector::Int8::View $vv --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_diagonal($vv.view, $!matrix);
}
method subdiagonal-view(Math::Libgsl::Vector::Int8::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_subdiagonal($vv.view, $!matrix, $k);
}
method superdiagonal-view(Math::Libgsl::Vector::Int8::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::Int8) {
  Math::Libgsl::Vector::Int8.new: vector => mgsl_matrix_char_superdiagonal($vv.view, $!matrix, $k);
}
sub int8-prepmat(*@array --> CArray[int8]) is export {
  my CArray[int8] $array .= new: @array».Int;
}
sub int8-array-mat(Block $bl, UInt $size1, UInt $size2, *@data) is export {
  my CArray[int8] $carray .= new: @data».Int;
  my Math::Libgsl::Matrix::Int8::View $mv .= new;
  my $m = $mv.array($carray, $size1, $size2);
  $bl($m);
}
sub int8-array-tda-mat(Block $bl, UInt $size1, UInt $size2, size_t $tda where * > $size2, *@data) is export {
  my CArray[int8] $carray .= new: @data».Int;
  my Math::Libgsl::Matrix::Int8::View $mv .= new;
...
method tricpy(Math::Libgsl::Matrix::Int8 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > v2.5 {
    ...
    $ret = gsl_matrix_char_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 0 !! 1, $!matrix, $src.matrix);
  }
...
method transpose-tricpy(Math::Libgsl::Matrix::Int8 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > v2.5 {
    ...
    $ret = gsl_matrix_char_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 0 !! 1, $!matrix, $src.matrix);
  }

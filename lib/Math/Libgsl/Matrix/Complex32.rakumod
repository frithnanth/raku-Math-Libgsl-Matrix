...
unit class Math::Libgsl::Matrix::Complex32:ver<0.6.1>:auth<zef:FRITH>;
...
multi method list(Math::Libgsl::Matrix::Complex32: --> List) {
  ...
  do for ^$!matrix.size1 {
    gsl_matrix_complex_float_get_row($v, $!matrix, $_);
    (^$v.size).map( { mgsl_vector_complex_float_get($v, $_, $c); $c.dat[0] + $c.dat[1] * i } ).eager;
  }
}
multi method gist(Math::Libgsl::Matrix::Complex32: --> Str) {
  ...
  do for ^$size1 {
    gsl_matrix_complex_float_get_row($v, $!matrix, $_);
    '(' ~ (^$size2).map({ mgsl_vector_complex_float_get($v, $_, $c); $c.dat[0] + $c.dat[1] * i }).Str ~ "$ellip1)\n";
  } ~ "$ellip2)"
}
multi method Str(Math::Libgsl::Matrix::Complex32: --> Str) { self.list.join(' ') }
# Accessors

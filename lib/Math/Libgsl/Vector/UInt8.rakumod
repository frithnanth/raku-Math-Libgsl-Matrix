...
unit class Math::Libgsl::Vector::UInt8:ver<0.5.0>:auth<zef:FRITH>;
...
multi method Str(Math::Libgsl::Vector::UInt8: --> Str) {
  (^$!vector.size).map({ gsl_vector_uchar_get($!vector, $_) })».Str.join(' ')
}
...
method sum(--> UInt) {
  ...
  gsl_vector_uchar_sum($!vector)
}
method axpby(UInt(Cool) $alpha, UInt(Cool) $beta, Math::Libgsl::Vector::UInt8 $b where $!vector.size == .vector.size) {
  X::Libgsl.new(errno => GSL_FAILURE, error => "Error in axpby: version < v2.7").throw if $gsl-version < v2.7;
  my $ret = gsl_vector_uchar_axpby($alpha, $!vector, $beta, $b.vector);

...
unit class Math::Libgsl::Matrix::Num32:ver<0.5.0>:auth<zef:FRITH>;
...
class View {
  ...
  method submatrix(Math::Libgsl::Matrix::Num32 $m, size_t $k1 where * < $m.size1, size_t $k2 where * < $m.size2, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::Num32) {
    fail X::Libgsl.new: errno => GSL_EDOM, error => "Submatrix indices out of bound" if $k1 + $n1 > $m.size1 || $k2 + $n2 > $m.size2;
    Math::Libgsl::Matrix::Num32.new: matrix => mgsl_matrix_float_submatrix($!view, $m.matrix, $k1, $k2, $n1, $n2);
  }
  method vector(Math::Libgsl::Vector::Num32 $v, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::Num32) {
    Math::Libgsl::Matrix::Num32.new: matrix => mgsl_matrix_float_view_vector($!view, $v.vector, $n1, $n2);
  }
  method vector-tda(Math::Libgsl::Vector::Num32 $v, size_t $n1, size_t $n2, size_t $tda where * > $n2 --> Math::Libgsl::Matrix::Num32) {
    Math::Libgsl::Matrix::Num32.new: matrix => mgsl_matrix_float_view_vector_with_tda($!view, $v.vector, $n1, $n2, $tda);
  }
  method array($array, UInt $size1, UInt $size2 --> Math::Libgsl::Matrix::Num32) {
    Math::Libgsl::Matrix::Num32.new: matrix => mgsl_matrix_float_view_array($!view, $array, $size1, $size2);
  }
  method array-tda($array, UInt $size1, UInt $size2, size_t $tda where * > $size2 --> Math::Libgsl::Matrix::Num32) {
    Math::Libgsl::Matrix::Num32.new: matrix => mgsl_matrix_float_view_array_with_tda($!view, $array, $size1, $size2, $tda);
  }
}
...
multi method list(Math::Libgsl::Matrix::Num32: --> List) {
  my gsl_vector_float $v = gsl_vector_float_calloc($!matrix.size2);
  LEAVE { gsl_vector_float_free($v) }

  do for ^$!matrix.size1 {
    gsl_matrix_float_get_row($v, $!matrix, $_);
    (^$v.size).map( { gsl_vector_float_get($v, $_) } ).eager;
  }
}
multi method gist(Math::Libgsl::Matrix::Num32: --> Str) {
  ...
  do for ^$size1 {
    gsl_matrix_float_get_row($v, $!matrix, $_);
    '(' ~ (^$size2).map({ gsl_vector_float_get($v, $_) }).Str ~ "$ellip1)\n";
  } ~ "$ellip2)"
}
multi method Str(Math::Libgsl::Matrix::Num32: --> Str) { self.list.join(' ') }
# Accessors
...
# View
method row-view(Math::Libgsl::Vector::Num32::View $vv, size_t $i where * < $!matrix.size1 --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_row($vv.view, $!matrix, $i);
}
method col-view(Math::Libgsl::Vector::Num32::View $vv, size_t $j where * < $!matrix.size2 --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_column($vv.view, $!matrix, $j);
}
method subrow-view(Math::Libgsl::Vector::Num32::View $vv, size_t $i where * < $!matrix.size1, size_t $offset, size_t $n --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_subrow($vv.view, $!matrix, $i, $offset, $n);
}
method subcol-view(Math::Libgsl::Vector::Num32::View $vv, size_t $j where * < $!matrix.size2, size_t $offset, size_t $n --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_subcolumn($vv.view, $!matrix, $j, $offset, $n);
}
method diagonal-view(Math::Libgsl::Vector::Num32::View $vv --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_diagonal($vv.view, $!matrix);
}
method subdiagonal-view(Math::Libgsl::Vector::Num32::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_subdiagonal($vv.view, $!matrix, $k);
}
method superdiagonal-view(Math::Libgsl::Vector::Num32::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::Num32) {
  Math::Libgsl::Vector::Num32.new: vector => mgsl_matrix_float_superdiagonal($vv.view, $!matrix, $k);
}
sub num32-prepmat(*@array --> CArray[num32]) is export {
  my CArray[num32] $array .= new: @array».Num;
}
sub num32-array-mat(Block $bl, UInt $size1, UInt $size2, *@data) is export {
  my CArray[num32] $carray .= new: @data».Num;
  my Math::Libgsl::Matrix::Num32::View $mv .= new;
  my $m = $mv.array($carray, $size1, $size2);
  $bl($m);
}
sub num32-array-tda-mat(Block $bl, UInt $size1, UInt $size2, size_t $tda where * > $size2, *@data) is export {
  my CArray[num32] $carray .= new: @data».Num;
  my Math::Libgsl::Matrix::Num32::View $mv .= new;

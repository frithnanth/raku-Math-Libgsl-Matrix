...
unit class Math::Libgsl::Matrix::Int16:ver<0.6.0>:auth<zef:FRITH>;
...
multi method list(Math::Libgsl::Matrix::Int16: --> List) {
  my gsl_vector_short $v = gsl_vector_short_calloc($!matrix.size2);
  LEAVE { gsl_vector_short_free($v) }

  do for ^$!matrix.size1 {
    gsl_matrix_short_get_row($v, $!matrix, $_);
    (^$v.size).map( { gsl_vector_short_get($v, $_) } ).eager;
  }
}
multi method gist(Math::Libgsl::Matrix::Int16: --> Str) {
  ...
  do for ^$size1 {
    gsl_matrix_short_get_row($v, $!matrix, $_);
    '(' ~ (^$size2).map({ gsl_vector_short_get($v, $_) }).Str ~ "$ellip1)\n";
  } ~ "$ellip2)"
}
multi method Str(Math::Libgsl::Matrix::Int16: --> Str) { self.list.join(' ') }
# Accessors

...
unit class Math::Libgsl::Matrix::Int64:ver<0.1.2>:auth<cpan:FRITH>;
...
sub mat-view-array(Math::Libgsl::Matrix::Int64::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export(:withsub) {
...
sub mat-view-array-tda(Math::Libgsl::Matrix::Int64::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export(:withsub) {
...
method tricpy(Math::Libgsl::Matrix::Int64 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  my $ret = gsl_matrix_long_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
...
method transpose-tricpy(Math::Libgsl::Matrix::Int64 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  fail X::Libgsl.new: errno => GSL_ENOTSQR, error => "Not a square matrix" if $!matrix.size1 ≠ $!matrix.size2;
  my $ret = gsl_matrix_long_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);

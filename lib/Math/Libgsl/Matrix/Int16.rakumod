...
unit class Math::Libgsl::Matrix::Int16:ver<0.1.4>:auth<cpan:FRITH>;
...
sub mat-view-array(Math::Libgsl::Matrix::Int16::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }) is export(:withsub) {
...
sub mat-view-array-tda(Math::Libgsl::Matrix::Int16::View $mv, @array where { @array ~~ Array && @array.shape.elems == 2 }, size_t $tda) is export(:withsub) {
...
sub mat-view-vector(Math::Libgsl::Matrix::Int16::View $mv, Math::Libgsl::Vector::Int16 $v, size_t $n1, size_t $n2) is export(:withsub) {
  Math::Libgsl::Matrix::Int16.new: matrix => mgsl_matrix_short_view_vector($mv.view, $v.vector, $n1, $n2);
}
sub mat-view-vector-tda(Math::Libgsl::Matrix::Int16::View $mv, Math::Libgsl::Vector::Int16 $v, size_t $n1, size_t $n2, size_t $tda) is export(:withsub) {
...
method tricpy(Math::Libgsl::Matrix::Int16 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_short_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }
...
method transpose-tricpy(Math::Libgsl::Matrix::Int16 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > 2.5 {
    ...
    $ret = gsl_matrix_short_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 1 !! 0, $!matrix, $src.matrix);
  }

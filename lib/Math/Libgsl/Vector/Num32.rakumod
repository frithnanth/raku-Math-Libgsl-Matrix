...
unit class Math::Libgsl::Vector::Num32:ver<0.6.0>:auth<zef:FRITH>;
...
multi method list(Math::Libgsl::Vector::Num32: --> List) { (^$!vector.size).map({ gsl_vector_float_get($!vector, $_) }).List }
multi method gist(Math::Libgsl::Vector::Num32: --> Str) {
  my ($size, $ellip) = $!vector.size > 100 ?? (100, ' ...') !! ($!vector.size, '');
  '(' ~ (^$size).map({ gsl_vector_float_get($!vector, $_) }).Str ~ "$ellip)";
}
multi method Str(Math::Libgsl::Vector::Num32: --> Str) { self.list.join(' ') }

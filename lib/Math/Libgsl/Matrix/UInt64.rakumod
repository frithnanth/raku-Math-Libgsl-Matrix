...
unit class Math::Libgsl::Matrix::UInt64:ver<0.4.1>:auth<zef:frithnanth>;
...
class View {
  ...
  method submatrix(Math::Libgsl::Matrix::UInt64 $m, size_t $k1 where * < $m.size1, size_t $k2 where * < $m.size2, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::UInt64) {
    fail X::Libgsl.new: errno => GSL_EDOM, error => "Submatrix indices out of bound" if $k1 + $n1 > $m.size1 || $k2 + $n2 > $m.size2;
    Math::Libgsl::Matrix::UInt64.new: matrix => mgsl_matrix_ulong_submatrix($!view, $m.matrix, $k1, $k2, $n1, $n2);
  }
  method vector(Math::Libgsl::Vector::UInt64 $v, size_t $n1, size_t $n2 --> Math::Libgsl::Matrix::UInt64) {
    Math::Libgsl::Matrix::UInt64.new: matrix => mgsl_matrix_ulong_view_vector($!view, $v.vector, $n1, $n2);
  }
  method vector-tda(Math::Libgsl::Vector::UInt64 $v, size_t $n1, size_t $n2, size_t $tda where * > $n2 --> Math::Libgsl::Matrix::UInt64) {
    Math::Libgsl::Matrix::UInt64.new: matrix => mgsl_matrix_ulong_view_vector_with_tda($!view, $v.vector, $n1, $n2, $tda);
  }
  method array($array, UInt $size1, UInt $size2 --> Math::Libgsl::Matrix::UInt64) {
    Math::Libgsl::Matrix::UInt64.new: matrix => mgsl_matrix_ulong_view_array($!view, $array, $size1, $size2);
  }
  method array-tda($array, UInt $size1, UInt $size2, size_t $tda where * > $size2 --> Math::Libgsl::Matrix::UInt64) {
    Math::Libgsl::Matrix::UInt64.new: matrix => mgsl_matrix_ulong_view_array_with_tda($!view, $array, $size1, $size2, $tda);
  }
}
...
# View
method row-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $i where * < $!matrix.size1 --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_row($vv.view, $!matrix, $i);
}
method col-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $j where * < $!matrix.size2 --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_column($vv.view, $!matrix, $j);
}
method subrow-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $i where * < $!matrix.size1, size_t $offset, size_t $n --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_subrow($vv.view, $!matrix, $i, $offset, $n);
}
method subcol-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $j where * < $!matrix.size2, size_t $offset, size_t $n --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_subcolumn($vv.view, $!matrix, $j, $offset, $n);
}
method diagonal-view(Math::Libgsl::Vector::UInt64::View $vv --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_diagonal($vv.view, $!matrix);
}
method subdiagonal-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_subdiagonal($vv.view, $!matrix, $k);
}
method superdiagonal-view(Math::Libgsl::Vector::UInt64::View $vv, size_t $k where * < min($!matrix.size1, $!matrix.size2) --> Math::Libgsl::Vector::UInt64) {
  Math::Libgsl::Vector::UInt64.new: vector => mgsl_matrix_ulong_superdiagonal($vv.view, $!matrix, $k);
}
sub uint64-prepmat(*@array --> CArray[uint64]) is export {
  my CArray[uint64] $array .= new: @array».Int;
}
sub uint64-array-mat(Block $bl, UInt $size1, UInt $size2, *@data) is export {
  my CArray[uint64] $carray .= new: @data».Int;
  my Math::Libgsl::Matrix::UInt64::View $mv .= new;
  my $m = $mv.array($carray, $size1, $size2);
  $bl($m);
}
sub uint64-array-tda-mat(Block $bl, UInt $size1, UInt $size2, size_t $tda where * > $size2, *@data) is export {
  my CArray[uint64] $carray .= new: @data».Int;
  my Math::Libgsl::Matrix::UInt64::View $mv .= new;
...
method tricpy(Math::Libgsl::Matrix::UInt64 $src where $!matrix.size1 == .matrix.size1 && $!matrix.size2 == .matrix.size2, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > v2.5 {
    ...
    $ret = gsl_matrix_ulong_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 0 !! 1, $!matrix, $src.matrix);
  }
...
method transpose-tricpy(Math::Libgsl::Matrix::UInt64 $src where $!matrix.size1 == .matrix.size2 && $!matrix.size2 == .matrix.size1, Int $Uplo, Int $Diag) {
  ...
  if $gsl-version > v2.5 {
    ...
    $ret = gsl_matrix_ulong_transpose_tricpy($Uplo == CblasUpper ?? 'U'.ord !! 'L'.ord, $Diag == CblasUnit ?? 0 !! 1, $!matrix, $src.matrix);
  }
